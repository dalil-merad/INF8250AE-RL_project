...
The paper positions DDQN as a **local planner** that uses **LiDAR + local targets** (waypoints) to follow a global path while reacting to obstacles in unknown environments. However, the reliance on waypoints implies the presence of some form of Global Path Planning, which contradicts the principles of path planning in unknown environments
...
This choice improves reproducibility when waypoint supervision or simulator details are missing. It is also more representative of real path planning in unknown environments, since our navigation is conditioned only on the final goal, such as when robots navigate toward a UWB beacon. However, this approach also makes the learning problem **more reward-sparse** unless the reward is shaped appropriately.
...
- `W` = wall
- `U` = obstacle that moves up and down
- `S` = obstacle that moves sideways
- `.` = free space
...
### Reproduced Maps (in order of difficulty)

Below are the three main maps used in our experiments, reproduced from the original paper and presented in order of increasing difficulty. All maps are shown below, side by side, at a smaller size for quick comparison:

<div align="center">
  <img src="assets/map_1.png" alt="Map 1" width="180"/>
  <img src="assets/map_2.png" alt="Map 2" width="180"/>
  <img src="assets/map_3.png" alt="Map 3" width="180"/>
</div>

<div align="center">
  <b>Map 1: Low Level</b> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <b>Map 2: Intermediate</b> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <b>Map 3: High Level</b>
</div>

**Map 1:** Mostly open space with a few large obstacles. Easiest scenario

**Map 2:** More obstacles, scattered throughout the environment, requiring more complex navigation and obstacle avoidance.

**Map 3:** Most challenging, featuring rooms and corridors that require the agent to plan around walls and navigate through narrow passages. Only used as holdout for evaluation.
...
Lei, Xiaoyun, et al. *Dynamic Path Planning of Unknown Environment Based on Deep Reinforcement Learning.* Journal of Robotics, vol. 2018, 18 Sept. 2018, pp. 1–10, https://doi.org/10.1155/2018/5781591. 
