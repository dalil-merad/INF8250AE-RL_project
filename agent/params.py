class Params:
    ...
    TARGET_UPDATE_FREQUENCY = 300  # Mise à jour du réseau cible tous les C pas
    ...
    L_MAX = 1.0  # Distance de départ max
    ...
    def as_dict(cls) -> dict:
        """Return all uppercase class attributes as a plain dict (JSON-serializable when possible)."""
        out = {}
        for k, v in vars(cls).items():
            if not k.isupper():
                continue
            # Keep it simple: JSON can handle numbers/strings/bools; other types will fail loudly.
            out[k] = v
        return out